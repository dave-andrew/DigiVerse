...
public class OfflineGame {
	...
	private void updateBullets(double deltaTime) {
		...
		while (bulletIterator.hasNext()) {
			...
			if (bullet.getPosX() < 0 || bullet.getPosX() > ScreenManager.SCREEN_WIDTH || bullet.getPosY() < 0 || bullet.getPosY() > ScreenManager.SCREEN_HEIGHT) {
			...
			while (enemyIterator.hasNext()) {
				...
				while (bulletIterator.hasNext()) {
					...
					if (bullet.getCollider().collidesWith(enemy.getCollider()) && !(enemy.getState() instanceof EnemyDeadState)) {
						...
						bulletIterator.remove();
					} else if (bullet.getPosX() < 0 || bullet.getPosX() > ScreenManager.SCREEN_WIDTH || bullet.getPosY() < 0 || bullet.getPosY() > ScreenManager.SCREEN_HEIGHT) {
						bullet.changeState(bullet.stopState);
						root.getChildren().remove(bullet);
						bulletIterator.remove();
					}
